...
// GET /api/articles/journalist/:journalistName
export async function getArticlesByJournalist(req, res) {
  try {
    const articles = await articleRepository.getArticlesByJournalist(req.params.journalistName);
    res.json(articles);
  } catch (error) {
    console.error('Error fetching journalist articles:', error);
    res.status(500).json({ message: "Failed to fetch journalist articles" });
  }
}
